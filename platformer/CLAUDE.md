...
- 셀 크기 16px. 타일셋(`Sprites/tilemap.tres`)의 모든 타일은 `buildTileTools()`가 `tile_x_y` 이름으로 자동 등록해 팔레트(5x3 격자, ◀ ▶ 페이지 넘김)에 나온다 — 지형(충돌)→장식(통과)→오브젝트 묶음 순 정렬이고 묶음마다 새 줄에서 시작. 이름·단축키가 필요한 타일만 `TILE_TYPES`와 `TOOL_INFO`에 항목 추가 (저장 포맷의 type 문자열이 이 이름이므로 기존 이름 변경 금지). 빈(전부 투명) 타일 칸은 자동으로 걸러진다
- 팔레트 아이콘은 게임 오브젝트가 실제로 쓰는 텍스처를 그대로 재사용한다(`toolIconTexture` — 타일은 아틀라스에서 잘라 쓰고, 시작 지점/초록 인간은 플레이어 스프라이트에 색만 입힘) — 타일셋이나 오브젝트 텍스처를 바꾸면 팔레트에 자동 반영된다
- 오브젝트 도구를 추가하면 `Tool` enum·`TOOL_INFO` 외에 `buildTileTools()`의 `objectTools` 목록에도 넣어 팔레트 자리를 정한다 (빠뜨려도 오브젝트 묶음 끝에 자동으로 붙는다)
...
1. 맵 에디터로 실제 레벨 제작 (`maps/map1.json`·`map2.json` 진행 중 — map1이 더 진행됨)
2. 완성된 맵을 실제 게임에서 불러와 진행하는 흐름 만들기 (게임 씬은 시작 씬 `start.tscn` 하나뿐이고, JSON 맵을 로드하는 쪽은 아직 에디터뿐이다)
